...
public class Ex01_Day04_Homework {
	public static void main(String[] args) {
		...
		//메뉴 변수 선언
		char menu; //초기값을 해야 하는 경우는? => for문, while문에서 조건식으로 들어갈 때
		//스캐너 선언 : 콘솔에서 입력받기 위해서
		Scanner scan = new Scanner(System.in);
		int max = 100, min = 1;
		int maxCount =0; //int가 가질 수 있는 최대값 max - min + 1라고 지정해도 됨 1~100 사이에선
		//반복문 : 종료를 선택할 때까지 반복
		do {
			//메뉴 출력
			...
			case '1':
				...
				do {
					...
					input = scan.nextInt();
					//입력한 값에 따라 결과를 출력
					//입력값이 랜덤보다 크면 Down, 작으면 Up, 같으면 정답을 출력
					if(input > random) {
				...
				}while(input != random);//랜덤한 정수와 입력한 수가 같으면 종료. 같지 않으면 반복.
			...
			case '2':
				...
				if (maxCount != 0) {
					System.out.println("최고 기록: " + maxCount + "회");
				}else {

...
public class Ex01_FunctionInterface {
	...
	public static void main(String[] args) {
		...
		//xx0101-x
		Person p = randomNum(()->{
			int year = (int)(Math.random()*(99 - 0 + 1) + 0); //0~99사이의 랜덤수 생성
			DecimalFormat df = new DecimalFormat("00"); //df로 앞의 두자리 00으로 포멧?
			String yearStr = df.format(year); 
			int gender = (int)(Math.random()*(4 - 1 + 1) + 1); //주민번호 뒤에서 첫자리는 1~4까지임

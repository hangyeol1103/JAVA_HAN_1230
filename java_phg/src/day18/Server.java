...
public class Server {
	...
	private void withdrawal() {
		try {
			...
			Account tmp = list.get(index);
			synchronized (tmp) {
				res = tmp.withdrawl(money) ? tmp.getMoney() : -1;
	...
	private void check() {
		try {
			...
			Account tmp = null;
			if(index >= 0) {
				tmp = (Account)list.get(index).clone(); //clone을 할 때는 리턴타입이 Object이기 때문에 Accout로 형 변환 해줘야 함.
			}
			oos.writeObject(tmp);
			oos.flush();

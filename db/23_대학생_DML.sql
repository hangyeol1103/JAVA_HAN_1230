...
WHERE
	DE_NAME = "컴퓨터공학과";
# 교수를 등록. 이름 김교수, 연락처 012-3311-5555, 주민번호 630101-2, 취임연도 2000. 컴퓨터공학과
INSERT INTO PROFESSOR
SELECT
	CONCAT("P", 2000, 160, LPAD(COUNT(PR_NUM) + 1, 3, "0")),
	"김교수", "012-3311-5555", "630101-2", 2000, "COM"
FROM
	PROFESSOR
WHERE
	PR_YEAR = 2000;
# 교수를 등록. 이름 박교수, 연락처 012-8795-4455, 주민번호 670331-1, 취임연도 2005. 디자인공학과
INSERT INTO PROFESSOR
SELECT
	CONCAT("P", 2000, DE_NAME_NUM, LPAD(COUNT(PR_NUM) + 1, 3, "0")),
	"박교수", "012-8795-4455", "670331-1", 2005, DE_NUM
FROM
...
WHERE
	DE_NAME = "디자인공학과";
# 교수를 등록. 이름 이교수, 연락처 012-9881-4415, 주민번호 701231-2, 취임연도 2010. 디자인공학과
INSERT INTO PROFESSOR
SELECT
	CONCAT("P", 2010, DE_NAME_NUM, LPAD(COUNT(PR_NUM) + 1, 3, "0")),
	"이교수", "012-9881-4415", "701231-2", 2010, DE_NUM
FROM
...
WHERE
	DE_NAME = "디자인공학과";
# 교수를 등록. 이름 최교수, 연락처 012-2234-3455, 주민번호 650101-1, 취임연도 2005. 기계공학과
INSERT INTO PROFESSOR
SELECT
	CONCAT("P", 2005, DE_NAME_NUM, LPAD(COUNT(PR_NUM) + 1, 3, "0")),
	"최교수", "012-2234-3455", "650101-1", 2005, DE_NUM
FROM
	(SELECT * FROM PROFESSOR WHERE PR_YEAR = 2005) P
RIGHT JOIN
	DEPARTMENT ON DE_NUM = PR_DE_NUM
WHERE
	DE_NAME = "기계공학과";
# 교수를 등록. 이름 최교수, 연락처 012-4534-2155, 주민번호 750101-1, 취임연도 2020. 기계공학과
INSERT INTO PROFESSOR
SELECT
	CONCAT("P", 2020, DE_NAME_NUM, LPAD(COUNT(PR_NUM) + 1, 3, "0")),
	"최교수", "012-4534-2155", "750101-1", 2020, DE_NUM
FROM
...
("나애리","013-1111-4444",2025,"060303-4","DEG"),
...
INSERT INTO SUBJECT(SJ_TITLE, SJ_POINT, SJ_TIME, SJ_CODE, SJ_SC_CODE)
SELECT "교육학 개론", 2, 2, CONCAT("TEA", LPAD(COUNT(*) + 1, 3, "0")), "TEA" FROM SUBJECT WHERE SJ_SC_CODE = "TEA";
INSERT INTO SUBJECT(SJ_TITLE, SJ_POINT, SJ_TIME, SJ_CODE, SJ_SC_CODE)
SELECT "교육 방법론", 2, 2, CONCAT("TEA", LPAD(COUNT(*) + 1, 3, "0")), "TEA" FROM SUBJECT WHERE SJ_SC_CODE = "TEA";
...
# 강의명 : 컴퓨터개론(1), 2025, 학기 : 1, 강의실 : KH 1관 501호, 분반 : 1, 정원 : 30, 강의계획서 : 없음, 교수 : 홍교수(P2000160001)
# 시간 : 월1A, 월1B, 월2A, 월2B
...
INSERT INTO LECTURE_SCHEDULE(LC_DAY, LC_TIME, LC_MINUTE, LC_LE_NUM) VALUES
("목", 1, "A", 4), ("목", 1, "B", 4), ("목", 2, "A", 4),
("목", 2, "B", 4), ("목", 3, "A", 4), ("목", 3, "B", 4);

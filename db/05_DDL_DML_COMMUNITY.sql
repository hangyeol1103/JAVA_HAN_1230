...
테이블명 : BOARD
 ...
 - 작성일은 년,월,일,시,분,초를 확인할 수boardboard 있다.
*/
DROP DATABASE IF EXISTS COMMUNITY; -- 전체실행시 꼬이는 부분을 DROP하고 다시 실행하기위해
...
CREATE TABLE BOARD(
    ...
    BO_DATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, -- CURRENT_TIMESTAMP는 기본값으로 현재 시간을 넣어줌
    BO_VIEW INT NOT NULL DEFAULT 0
);
# abc123회원이 "인사"라는 제목으로 "안녕하세요"라는 내용을 작성했을 때 필요한 쿼리
INSERT INTO BOARD(BO_TITLE, BO_CONTENT, BO_WRITER) VALUES("인사", "안녕하세요", "abc1234");
INSERT INTO BOARD(BO_TITLE, BO_CONTENT, BO_WRITER) VALUES("공지", "공지입니다.", "ADMIN");
...
# 최시글을 조회하는 쿼리 => 등록된 날짜가 최근 => 날짜 순으로 정렬

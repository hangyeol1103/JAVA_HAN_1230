...
#	- AS 생략 가능
#	- 검색 결과가 실행되는 동안만 학년, 반, 번호, 이름으로 표시되는 것
#	- 학 년 처럼 띄어쓰기가 있을 시에 ""를 써야함.
SELECT ST_GRADE AS "학 년", ST_CLASS AS 반, ST_NUM 번호, ST_NAME "이름" FROM STUDENT;

# SELECT [DISTINCT] 속성1, ..., 속성N FROM 테이블명 [WHERE 조건식]
# 조건식을 만족하는 속성들을 조회하는 쿼리.
# 중복된 행을 제거 => 기본키 속성을 포함하지 않은 경우 중복이 가능
# 논리 연산자
#	- AND, OR, NOT
#	- AND의 우선순위가 OR보다 높다
# 비교 연산자
...
# 등록된 학생들의 학년을 조회하는 쿼리 //중복이 발생하는 상황이 발생할 때 DISTINCT 이용 
SELECT DISTINCT ST_GRADE 학년 FROM STUDENT;

# 1학년 학생들을 조회하는 쿼리
SELECT * FROM STUDENT WHERE ST_GRADE = 1;

# 1학년 1반의 번호가 1 ~ 3 사이의 학생들을 조회하는 쿼리
...
# 이 때 2페이지에서 조회 가능한 학생들을 선택하는 쿼리
...
-- 학년, 반이 같은 애들끼리 묶어서 개수를 세겠다. 그걸 그룹 바이로 묶어놔야 실행이 됨. 안묶어 놓으면 다 다르기 때문에.

# 각학년별 학생수를 조회
SELECT ST_GRADE, COUNT(*) AS "학생수" FROM STUDENT.STUDENT GROUP BY ST_GRADE;

# 학생수가 5명 이상인 학년들을 조회 `학생 수`를 역따옴표로 한 이유는 문자열이 아닌 속성으로 쓰기 위해
...
/*
- 테이블A가 테이블B에 참조되고 있으면 테이블B에는 외래키, 테이블 A에는 기본키로 연결이 되어 있을 때 JOIN을 사용
...
/*
SELECT * FROM 테이블A JOIN 테이블B ON 테이블B.속성1 = 테이블A.속성1; 을 밑의줄로 수정 가능.
...
# 성적이 90이상이면 A, 80이상이면 B, 70이상이면 C, 60이상이면 D, 60미만이라면 F로 출력하는 쿼리
SELECT *, 
	CASE
...
# ST_GRADE를 그룹바이 할 때 학년을 제외한 나머지 값들은 키값이 다르기 때문에 에러 발생.
# 따라서 밑에 줄로 세팅시에 에러를 해결할 수 있다.
...
SELECT ST_GRADE 학년, ST_CLASS 반, ST_NUM 번호, ST_NAME 이름, IFNULL(AVG(SC_SCORE), 0) 평균
	FROM SCORE
RIGHT JOIN
	STUDENT ON SC_ST_KEY = ST_KEY -- STUDENT를 기준으로 작성할 건데, JOIN을 기준으로 오른쪽에 있으니 RIGHT JOIN을 쓴 것.
GROUP BY ST_KEY;

# 1학년 1반 반 등수를 조회하는 쿼리(평균). 평균이 같으면 국어, 영어, 수학 점수 순으로 비교하여 등수를 결정. 다 같으면 같은 등수

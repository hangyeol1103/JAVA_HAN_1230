...
/*
학생 등록/수정/삭제
  - 학생의 전공, 이름, 학년, 연락처, 주민번호를 입력하여 학생을 등록한다. 학생에서 기본키 주민번호로.
  - 학번은 직접 입력하지 않고, 입학년도 4자리, 전공코드 3자리, 이름순 3자리를 이용하여 구성된다. 전공코드 테이블 필요. 학생/교수
  - 학생은 등록되기 전 학생 이름, 전공, 주민번호가 이미 등록되어 있다.(입학 원서) - 입학 원서를 만들고 학생에 외래키로 지정
  - 주민번호는 14자리이다.
  - 전공은 전공 코드를 입력한다.
전공 등록/수정/삭제
...
과목 등록/수정/삭제 (ERD 짤 때 참고, 같은 교수님이 여러 과목을 수업할 수 있음.)
  - 과목코드, 과목명, 이수 학점, 시간을 입력하여 등록한다.
  - 과목코드는 분류코드3자리, 순서 3자리로 구성된다.
  - 분류코드는 교직(TEA), 교양(CUL), 전공별로 할당된 코드를 사용한다 

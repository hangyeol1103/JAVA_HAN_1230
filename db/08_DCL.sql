...
#	- SELECT/INSERT/UPDATE/DELETE/CREATE/ALTER/DROP/REFERENCES/ALL PRIVILEGES(모든 권한) privilege : 특권
# GRANT 권한 ON DB명.테이블명 TO '아이디'@'호스트명';
GRANT ALL PRIVILEGES ON COMMUNITY.* TO 'community_admin'@'%'; -- grant : 승인하다

# 권한 제거
# REVOKE 권한 ON DB명.테이블명 FROM '아이디'@'호스트명';
REVOKE INSERT,UPDATE ON COMMUNITY.* FROM 'community_admin'@'%'; -- revoke : 취소

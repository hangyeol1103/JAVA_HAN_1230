...
USE STUDENT;
INSERT INTO STUDENT VALUE(1, 1, 1, 1, "홍길동"); -- 기본키 입력했기에 충돌이 일어남. (중복)
...
INSERT INTO STUDENT(ST_GRADE, ST_CLASS, ST_NUM, ST_NAME) VALUE(1, 1, 2, "임꺽정"); -- 기본키 입력을 안했기에 충돌 x
INSERT INTO STUDENT(ST_NUM, ST_NAME) VALUE(3, "고길동"); -- 기본키 입력을 안했기에 충돌 x

# INSERT INTO 테이블명(속성1, ..., 속성N) VALUES(값1, ..., 값N), (값1, ..., 값N), ...;
INSERT INTO STUDENT(ST_NUM, ST_NAME) VALUES(4, "둘리"), (5, "하하");

# INSERT INTO 테이블명(속성1, ..., 속성N) SELECT 속성1, 값2, ..., 속성A FROM 테이블명 WHERE 조건
#	- 테이블에서 검색된 값을 이용하여 추가할 때 사용
#	- 여기서 값1, 값2.. 들은 지정된 값이고, 속성A는 검색 결과 값 

# 데이터 수정
# UPDATE 테이블명 SET 속성1 = 값, ..., 속성N = 값N WHERE 조건
# 조건에서 같다는 =, 같지 않다는 != 또는 <>
# NULL과 같다는 IS NULL, NULL과 같지 않다면 S NOT NULL
...
(6, 4, 60), (6, 5, 50), (6, 6, 100), # 2학년 1반 1번 학생 1학년 2학기 국영수 성적
(7, 4, 100), (7, 5, 40), (7, 6, 100), # 2학년 1반 2번 학생 1학년 2학기 국영수 성적
(8, 4, 100), (8, 5, 90), (8, 6, 80), # 2학년 1반 3번 학생 1학년 2학기 국영수 성적
(9, 4, 100), (9, 5, 30), (9, 6, 50), # 2학년 2반 1번 학생 1학년 2학기 국영수 성적
(10, 4, 40), (10, 5, 32), (10, 6, 100), # 2학년 2반 2번 학생 1학년 2학기 국영수 성적
(11, 4, 70), (11, 5, 53), (11, 6, 15), # 2학년 2반 3번 학생 1학년 2학기 국영수 성적
(12, 4, 100), (12, 5, 66), (12, 6, 74), # 2학년 3반 1번 학생 1학년 2학기 국영수 성적
(13, 4, 100), (13, 5, 43), (13, 6, 13), # 2학년 3반 2번 학생 1학년 2학기 국영수 성적
(14, 4, 64), (14, 5, 100), (14, 6, 100), # 2학년 3반 3번 학생 2학년 1학기 국영수 성적
(15, 7, 100), (15, 8, 54), (15, 9, 68); # 3학년 1반 1번 학생 2학년 1학기 국영수 성적

# 1학년 1반 1번의 1학년 2학기 국어 성적을 100점으로 추가할 때 쿼리
SELECT ST_KEY FROM STUDENT WHERE ST_GRADE = 1 AND ST_CLASS = 1 AND ST_NUM = 1;
SELECT * FROM SUBJECT WHERE SJ_GRADE = 1 AND SJ_SEMESTE = 2 AND SJ_NAME = "국어";
SELECT ST_KEY, SJ_NUM
FROM STUDENT
JOIN SUBJECT # 둘은 참조되어있지 않기에 JOIN으로 연결 후
WHERE -- WHERE에서 각각의 조건을 넣어줘서 연결시켜줌.
	ST_GRADE = 1 AND ST_CLASS = 1 AND ST_NUM = 1
    AND SJ_GRADE = 1 AND SJ_SEMESTER = 2 AND SJ_NAME = "국어";
    
SELECT ST_KEY,SJ_NUM
FROM (SELECT ST_KEY FROM STUDENT WHERE ST_GRADE = 1 AND ST_CLASS = 1 AND ST_NUM = 1) AS T1 # 그대로 실행시에는 안됨 따라서 AS로 이름을 지정해줘야함.
